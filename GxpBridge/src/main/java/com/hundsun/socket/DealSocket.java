    ...
    public void run() {
        ...
        try {
            while (socket!=null && socket.isConnected() && (!socket.isClosed())) {
                ...
                log.debug("读入 socket 发送过来的数据成功{}", reqJson);
                log.debug("开始发送 https");
                ResponseEntity response = sendHttpsAndGetRes(reqJson);
                String respJson = CommonUtil.objectToJson(response);
                log.debug("将结果数据{}写回socket", respJson);
                writeJson(respJson);
            }
    ...
    private void writeJson(String respJson) {
        if (socket != null && socket.isConnected() && !socket.isClosed()) {
            try {
                ...
                throw new RuntimeException("将数据{}返回给 gxp 失败", e);
            }
        } else {
            log.error("客户端 socket 已经关闭，无法将数据{}写h回客户端", respJson);
            throw new RuntimeException("客户端 socket 已经关闭，无法将数据写会客户端");
        }
    }
}
